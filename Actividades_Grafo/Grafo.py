...
class Graph:
    ...
    def dijkstra_con_destino(self, origen, destino):
        from math import inf
        no_visitados = HeapMin()
        camino = Stack()
        predecesores = {}  
        ...
        for adjacente in adjacentes:
                pos = no_visitados.search(adjacente['value'])
                if pos is not None:
                    nuevo_costo = costo_nodo_actual + adjacente['peso']
                    if nuevo_costo < no_visitados.elements[pos][0]:
                        no_visitados.elements[pos][1][2] = nodo_actual
                        no_visitados.change_proirity(pos, nuevo_costo)
                        predecesores[adjacente['value']] = nodo_actual
        nodo_camino = destino
        camino_corto = []
        while nodo_camino is not None:
                camino_corto.insert(0, nodo_camino)
                nodo_camino = predecesores[nodo_camino]
        if camino_corto[0] != origen:
                return float('inf'), []  
        return costo_nodo_actual, camino_corto
