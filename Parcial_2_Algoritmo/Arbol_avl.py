...
class BinaryTree:
    ...
    def contar_acero(self):
        def __contar_acero(root):
            counter = 0
            if root is not None:
                if root.value == 'Acero':
                    counter = 1
                counter += __contar_acero(root.left)
                counter += __contar_acero(root.right)
            return counter

        return __contar_acero(self.root)
    
    def contar_electrico(self):
            def __contar_electrico(root):
                counter = 0
                if root is not None:
                    if root.value == 'Eléctrico':
                        counter = 1
                    counter += __contar_electrico(root.left)
                    counter += __contar_electrico(root.right)
                return counter

            return __contar_electrico(self.root)
    ...
    def inorden_pokemon(self):
        def __inorden_pokemon(root):
            if root is not None:
                __inorden_pokemon(root.left)
                if root.value.get('tipos') is not None:
                    print(root.value)
                __inorden_pokemon(root.right)

        if self.root is not None:
            __inorden_pokemon(self.root)
