from Lista import by_names, by_specie
jedis = [
    {
        "name": "Qui-Gon Jinn",
        "rank": "Jedi Master",
...
Listado_nombre = jedis.sort(key=by_names)
# Listado_especie = jedis.sort(key=by_specie)
...
for jedi in jedis:
    ...
    if jedi["master"] == "Luke Skywalker":
        lista_luke.append(jedi["name"])
    if jedi["master"] == "Yoda":
        lista_yoda.append(jedi["name"])
    if jedi["species"] == "Human":
        humanos.append(jedi["name"])
    if jedi["species"] == "Twi'lek":
        twilek.append(jedi["name"])
    if jedi["name"][0] == "A":
        nombre_a.append(jedi["name"])
    if jedi["master"] == "Qui-Gon Jin":
        lista_quin.append(jedi["name"])
    if jedi["master"] == "Mace Windu":
        lista_mace.append(jedi["name"])
    if jedi["rank"] == "Grand Master":
        lista_rank.append(jedi["name"])
    if jedi["lightsaber_color"] == "Purple" or jedi["lightsaber_color"] == "Yellow":
        lista_light.append(jedi["name"])



print(f"""
Los padawan de Yoda son: {lista_yoda}
Los padawan de Luke son: {lista_luke}

Humanos: {humanos}
Twi'lek: {twilek}

Los jedis que comienzan con A, son: {nombre_a}

Los jedis que usan sable amarillo o violeta son: {lista_light}

Los padawan de Mace Windu son: {lista_mace}
Los padawan de Qui-Gon Jin son: {lista_quin}

Los jedis con rango Grand Master son: {lista_rank}""")